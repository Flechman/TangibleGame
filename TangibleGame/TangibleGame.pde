...
void drawGame() {
  ...
  if (!editMode) {
    ...
    if (!ps.particles.isEmpty()) {
      gameSurface.pushMatrix();
      gameSurface.rotateX(PI/2);
      //gameSurface.translate(testX, testY, -50);
      //gameSurface.translate(scalingFactor*(mouseX-gameSurface.width/2), scalingFactor*(-mouseY+gameSurface.height/2), -50);
